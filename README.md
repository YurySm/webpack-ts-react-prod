...
- `npm run build:prod:vite` - Сборка в prod режиме vite
...
- `npm run test:ui:ci:concurrently` - Запуск скриншотных тестов в CI c параллельным запуском storybook
- `npm run test:ui:report` - Генерация полного отчета для скриншотных тестов
- `npm run test:ui:json` - Генерация json отчета для скриншотных тестов
- `npm run test:ui:html` - Генерация HTML отчета для скриншотных тестов
- `npm run test:e2e` - Запуск e2e тестов на cypress 
...
используется собственный eslint plugin *@yury_sm/eslint-plugin-fsd-path-checker*,
...
Запросы на сервер отправляются с помощью [RTK query](/@/shared/api/rtkApi.ts)

Для асинхронного подключения редюсеров (чтобы не тянуть их в общий бандл) используется
[DynamicModuleLoader](/@/shared/lib/components/DynamicModuleLoader/DynamicModuleLoader.tsx)
...
```bash
    npx ts-node ./scripts/remove-feature.ts featureFlag on
```
